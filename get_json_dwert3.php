...
$query = mysqli_query($con, "SELECT ROUND(AVG(celsius) ,2) AS celsius,DATE_FORMAT(event,'%M') AS event FROM outdoortemp1 WHERE YEAR(event) = 2020 GROUP BY MONTH(event),YEAR(event) ORDER BY MONTH(event) ASC");
