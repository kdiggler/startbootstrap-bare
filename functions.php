...
function tempindoor()
{
   ...
   $query = "SELECT MAX(celsius) AS celsius, event, DATE(event) AS date_created FROM temperature GROUP BY DATE(event) ORDER BY celsius desc LIMIT 5";
...
function tempindoor_avg_pm()
{
  ...
  $query = "SELECT ROUND(AVG(celsius) ,2) AS celsius,DATE_FORMAT(event,'%M - %Y') AS event FROM temperature GROUP BY MONTH(event) ORDER BY event DESC";
